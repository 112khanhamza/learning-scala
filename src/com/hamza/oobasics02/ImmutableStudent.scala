...
class ImmutableStudent(
    ...
    def addQuiz(grade: Int): ImmutableStudent = if (validGrade(grade)) new ImmutableStudent(
        firstName,
        lastName,
        grade :: quizzes,
        assignments,
        tests)
    else this // if grade is not valid then return the current object

    def addAssignment(grade: Int): ImmutableStudent = if (validGrade(grade)) new ImmutableStudent(
        firstName,
        lastName,
        quizzes,
        grade :: assignments,
    ...
    def addTest(grade: Int): ImmutableStudent = if (validGrade(grade)) new ImmutableStudent(
        firstName,
        lastName,
        quizzes,
        assignments,
    ...
    def average: Double = quizAverage * 0.1 + assignmentAverage * 0.5 + testAverage * 0.40;
}
